...
string teststri = "The quick brown Fox jumps over the lazy Dog";
...
Console WriteLine(Caphords.IsMatch(teststr1));
Console WriteLine(Caphords.IsMatch(teststr2));
...
MatchCollection mc = CapWords Matches (teststri);
Console WriteLine($"' {m.Value}' found at position {m.Index}");
m = m.NextMatch();
